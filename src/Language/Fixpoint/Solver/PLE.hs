...
instantiate cfg fi' subcIds = do
    ...
    res   <- withRESTSolver $ \solver -> withProgress (1 + M.size cs) $
               withCtx cfg file sEnv $ \ctx ->
                  instEnv cfg fi cs solver ctx >>= pleTrie t                -- 2. TRAVERSE Trie to compute InstRes
...
data EvalEnv = EvalEnv
  ...
  , evOCAlgebra :: OCAlgebra OCType Expr IO
  }
...
evalApp γ _ctx e0 (e1:es2) _et
  | eqs@(_:_) <- nonUserDataMeasureEqs γ (eApps e0 (e1:es2))
  = do
       modify $ \st ->
         st { evNewEqualities = foldr S.insert (evNewEqualities st) eqs }
       return (Nothing, noExpand)
...
knowledge cfg ctx si = KN
  ...
  , knSimsByMeasureName        = Map.fromListWith (\(rw0, dms) (rw1, _) -> (rw0 ++ rw1, dms)) $
                                   ...
                                   , not $ isEVar (smBody rw) -- not a selector
                                   ] ++
